 ...
 * @brief Objective-C VolcEngine RTC Defines
*/
...
/**
 * @brief 用户需要透传的额外的信息，字符长度限制为 200 字节。会在 rtcEngine:onUserJoined:elapsed:{@link #rtcEngine:onUserJoined:elapsed:} 中回调给远端用户。
 */
 ...
 * @type keytype
 * @brief 音频播放设备类型。  <br>
 *        SDK 通过 rtcEngine:onAudioPlaybackDeviceChanged:{@link #rtcEngine:onAudioPlaybackDeviceChanged:} 回调当前音频设备播放类型。
 */
