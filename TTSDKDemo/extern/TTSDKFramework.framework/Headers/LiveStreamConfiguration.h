...
typedef NS_ENUM(NSInteger, LiveStreamMode)
{
    ...
    LiveStreamModeAudioANDDarkFrame = 3, //音频 + dark frame （用于音频直播）
};

typedef NS_ENUM(NSUInteger, LiveStreamVideoCodec) {
    LiveStreamVideoCodec_VT_264 = 0, // 硬编 264
    LiveStreamVideoCodec_VT_ByteVC1 = 1, // 硬编 ByteVC1
};
...
/**
  设置是否支持蓝牙耳返，默认YES，支持蓝牙耳返的前提是enableInEarMonitoring =YES
 */
...
/**
 音频编码类型
 默认为 LiveStreamAudioCodec_AT_AAC_LC
 */
...
/**
 ...
 ByteVC1 默认为 LiveEnCodecByteVC1MainAutoLevel
 */
@property (nonatomic, assign) LiveProfileLevelType profileLevel;

/**
 是否开启Extension模式, 默认为关闭
 */
