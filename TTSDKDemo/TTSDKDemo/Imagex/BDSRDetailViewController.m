...
- (void)viewDidLoad {
    ...
    [_imageView1 bd_setImageWithURL:[NSURL URLWithString:_url] placeholder:nil options:BDImageRequestIgnoreCache completion:^(BDWebImageRequest *request, UIImage *image, NSData *data, NSError *error, BDWebImageResultFrom from) {
        label1.text = [label1.text stringByAppendingFormat:@"(size:%.0fx%.0f)", image.size.width*image.scale, image.size.height*image.scale];
    ...
    [_imageView2 bd_setImageWithURL:[NSURL URLWithString:_url] placeholder:nil options:BDImageRequestIgnoreCache transformer:[BDSuperResolutionTransformer new] progress:NULL completion:^(BDWebImageRequest *request, UIImage *image, NSData *data, NSError *error, BDWebImageResultFrom from) {
        label2.text = [label2.text stringByAppendingFormat:@"(size:%.0fx%.0f)", image.size.width*image.scale, image.size.height*image.scale];
...
- (void)clickImage1:(UIView *)sender {
//    BDImageConfigShowViewController *vc = [BDImageConfigShowViewController new];
//    vc.navigationItem.title = @"图片日志详情";
//    vc.config = [_request1 requestRecoder];
//    [self.navigationController pushViewController:vc animated:YES];
}

- (void)clickImage2:(UIView *)sender {
//    BDImageConfigShowViewController *vc = [BDImageConfigShowViewController new];
//    vc.navigationItem.title = @"图片日志详情";
//    vc.config = [_request2 requestRecoder];
//    [self.navigationController pushViewController:vc animated:YES];
}
